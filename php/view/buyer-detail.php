...
<head>
    ...
    <title>Podrobnosti o kupcu</title>
</head>
<body>
    ...
    <form class="edit-form" action="<?= BASE_URL . "buyers" ?>" method="post">
        ...
        <div class="form-group">
            <label>🏠 Ulica:</label>
            <input type="text" name="address_street" value="<?= isset($user["address_street"]) ? $user["address_street"] : '' ?>" />
        </div>
        
        <div class="form-group">
            <label>🔢 Hišna številka:</label>
            <input type="text" name="address_number" value="<?= isset($user["address_number"]) ? $user["address_number"] : '' ?>" />
        </div>
        
        <div class="form-group">
            <label>🏙️ Kraj:</label>
            <input type="text" name="address_post" value="<?= isset($user["address_post"]) ? $user["address_post"] : '' ?>" />
        </div>
        
        <div class="form-group">
            <label>📮 Poštna številka:</label>
            <input type="text" name="address_zip" value="<?= isset($user["address_zip"]) ? $user["address_zip"] : '' ?>" />
        </div>
        ...
        <div class="address-display">
            <strong>📦 Celoten naslov:</strong>
            <?php
            $naslov_parts = array();
            if (isset($user["address_street"]) && !empty($user["address_street"])) {
                $naslov_parts[] = $user["address_street"];
            }
            if (isset($user["address_number"]) && !empty($user["address_number"])) {
                $naslov_parts[] = $user["address_number"];
            }
            if (isset($user["address_post"]) && !empty($user["address_post"])) {
                $naslov_parts[] = $user["address_post"];
            }
            if (isset($user["address_zip"]) && !empty($user["address_zip"])) {
                $naslov_parts[] = $user["address_zip"];
            }
            
            if (!empty($naslov_parts)) {
                echo implode(' ', $naslov_parts);
            } else {
                echo 'Ni podanega naslova';
            }
            ?>
        </div>
    ...
    <div class="back-link">
        <a href="<?= BASE_URL . "buyers" ?>">🎅 Nazaj na seznam kupcev 🎅</a>
    </div>

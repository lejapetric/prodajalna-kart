...
* Datoteke s koncnico `.pfx` vsebujejo osebne certifikate. Uvozite jih v
  brskalnik. Geslo za uvoz je `ep`.

* Datoteki `localhost.crt` ter `localhost.key` predstavljata strežnikov
...
* Datoteka `epca.crt` predstavlja certifikat certifikatne agencije EPCA,
  ki je podpisala vse certifikate. Njeno lokacijo najavite strežniku Apache2 v
  konfiguracijski datoteki z ukazom `SSLCACertificateFile /pot/do/epca.crt`.

  Prav tako morate ta certifikat uvoziti v brskalnik kot zaupanja vredno
